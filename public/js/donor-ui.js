(function () {
    const sidebar = document.getElementById('dnSidebar');
    const overlay = document.getElementById('dnOverlay');
    const hamburger = document.getElementById('dnHamburger');
    const pageTitle = document.getElementById('dnPageTitle');
    const links = document.querySelectorAll('[data-dn-section]');
    ...
    function showSection(id, label) {
        ...
        links.forEach(function (link) {
            link.classList.toggle('active', link.dataset.dnSection === id);
        });
        ...
        if (id === 'notifications' && typeof window.dnClearMessageAlerts === 'function') {
            window.dnClearMessageAlerts();
        }
    }
    ...
    links.forEach(function (link) {
        link.addEventListener('click', function (e) {
            e.preventDefault();
            showSection(link.dataset.dnSection, link.dataset.dnLabel || link.textContent.trim());
            history.replaceState(null, '', '#section-' + link.dataset.dnSection);
        });
    });

    const hash = window.location.hash.replace('#section-', '');
    if (hash) {
        const match = document.querySelector('[data-dn-section="' + hash + '"]');
        if (match) showSection(hash, match.dataset.dnLabel);
    }
    ...
    window.dnShowSection = showSection;
    ...
    document.querySelectorAll('[data-req-view]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.querySelectorAll('[data-req-view]').forEach(function (b) {
                b.classList.toggle('btn-success', b.dataset.reqView === view);
    ...
    document.getElementById('ntMarkRead')?.addEventListener('click', function () {
        if (typeof window.dnClearMessageAlerts === 'function') {
            window.dnClearMessageAlerts();
        }
    });
    ...
    function applyMsgFilters() {
        ...
        msgItems().forEach(function (card) {
            ...
            card.classList.toggle('dn-msg-hidden', !show);
            if (show) visible += 1;
        });
        if (msgVisibleCount) msgVisibleCount.textContent = String(visible);
        const msgBoardEl = document.getElementById('msgBoard');
        if (msgEmptyFilter) {
            const hasItems = msgItems().length > 0;
            const showEmpty = hasItems && visible === 0;
            msgEmptyFilter.classList.toggle('dn-msg-hidden', !showEmpty);
            msgEmptyFilter.classList.toggle('d-none', !showEmpty);
        }
        if (msgBoardEl) {
            const hasItems = msgItems().length > 0;
            msgBoardEl.classList.toggle('dn-msg-hidden', hasItems && visible === 0);
        }
    }

    function setMsgPillSelected(container, attr, value) {
        if (!container) return;
        container.querySelectorAll('.dn-when-pill').forEach(function (pill) {
            const selected = pill.dataset[attr] === value;
    ...
    document.getElementById('msgWhenFilters')?.querySelectorAll('.dn-when-pill').forEach(function (pill) {
        pill.addEventListener('click', function () {
            msgWhenFilter = pill.dataset.msgWhen;
            setMsgPillSelected(document.getElementById('msgWhenFilters'), 'msgWhen', msgWhenFilter);
            applyMsgFilters();
        });
    });
    ...
    document.getElementById('msgClearFilters')?.addEventListener('click', function () {
        ...
        setMsgPillSelected(document.getElementById('msgWhenFilters'), 'msgWhen', 'all');
        applyMsgFilters();
        if (typeof lucide !== 'undefined') lucide.createIcons();
    });
    ...
    document.querySelectorAll('.buy-plan-btn').forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            window.RaktaSetuPayments.openCheckout({ purpose: 'subscription', plan }, '#059669');
        });
    });
